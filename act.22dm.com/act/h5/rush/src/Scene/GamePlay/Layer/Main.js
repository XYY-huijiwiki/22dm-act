var GPMainLayer = cc.Layer.extend({
  ...
  loadRole: function () {
    ...
    let double = new cc.Sprite(`#role_${v_main.roleId}_2.png`);
  ...
  loadBridge: function (endX) {
    ...
    for (let i = 0; i < setting.length; i++) {
      let tmp = new cc.Sprite("#road.png");
      tmp.attr({
        anchorX: 0,
        anchorY: 0,
        x: i * (tmp.width - GameManager.bridgeOffsetX),
        y: 0,
        name: "bridge"
      });
      bridge.push(tmp);
    ...
    road.attr({
      width: bridge[bridge.length - 1].x + bridge[bridge.length - 1].width,
      x: setting.x,
      y: setting.y,
      setting: setting
    })
  ...
  checkBridgeCollide: function () {
    if (this.bridge.length > 0) {
      let lastbridge = this.bridge[this.bridge.length - 1];
      if (lastbridge.getBoundingBoxToWorld().x < winSize.width) {
        this.bridge = [];
        this.loadBridge(lastbridge.getBoundingBoxToWorld().x + lastbridge.width);
      }
    }
  },
  onRoleRush: function () {
