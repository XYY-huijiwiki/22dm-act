const sleep = async (e) => new Promise((t) => setTimeout(t, e));
...
var heartbeat = {
  ...
  ercodeShow: function () {
    ...
    $("#ercode").empty();
    var flag = encodeURIComponent(
      "scene=" + heartbeat.scene + "&" + "roomId=" + heartbeat.roomId
    );
  ...
  shareShow: function () {
    ...
    $("#dialogs_share")
      .addClass("share_scene_" + heartbeat.scene)
      .show();
    if (heartbeat.a == "share") {
      //打开的是分享链接
      $("#share_to").attr("disabled", "disabled");
      $("#share_from").attr("readonly", "readonly");
      $("#share_bg")
        .attr("src", "res/share_bg_" + heartbeat.scene + ".jpg")
        .show()
        .next()
        .show();
    } else
  ...
  loadResFinish() {
    ...
    setTimeout(function () {
      $("#" + heartbeat.loadtag + " .dialogs_precent")
        .hide()
        .next()
        .fadeIn();
    }, 500);
    if (heartbeat.a == "player_two") {
      cc.director.runScene(
        new GamePlayScene(heartbeat.scene, heartbeat.player)
      );
      dialogs.close();
    }
  },
            ...
            case 3: //玩家二加载场景完毕 播放动画 获取玩家2的名字
              ...
              if (heartbeat.scene == 1)
                cc.eventManager.dispatchEvent(
                  new cc.EventCustom(jf.EventName.GP_FEMALE_A_3)
                );
              else
                cc.eventManager.dispatchEvent(
                  new cc.EventCustom(jf.EventName.GP_MALE_A_2)
                );
              break;
          }
        }
...
var player_two = {
  getStatu: async function () {
    ...
    if (heartbeat.scene == 1) {
      cc.eventManager.dispatchEvent(
        new cc.EventCustom(jf.EventName.GP_FEMALE_A_4)
      );
      console.log(
        "cc.eventManager.dispatchEvent(new cc.EventCustom(jf.EventName.GP_FEMALE_A_4));"
      );
      ...
      cc.eventManager.dispatchEvent(
        new cc.EventCustom(jf.EventName.GP_MALE_A_3)
      );
    }
  },
  sendStatu: function () {
    //发送状态 扫码进入发送2  加载完成发送3
    $.ajax({
      cache: false,
      async: false,
      type: "POST",
