// 动作管理对象
var ActionManager = {
    GP_ROPE_RUN : function(ropeType,speedX,speedY,direction,callback){
        var deltaX = direction==0 ? -winSize.width+100 : winSize.width-100;
        var moveX = cc.moveBy(speedX,deltaX,0);
        var moveY = cc.moveBy(speedY,0 ,-winSize.height-300);
        var cb = cc.callFunc(function(event){
            callback();
        });
        return cc.sequence(cc.spawn(moveX,moveY),cb);
    },
    GP_CAR_BREAK : function(ropeType,delay,callback){
        var frames = [];
        for(var i=1;i<=40;i++){
            var str = "car_"+ropeType+"_"+i+".png";
    ...
    GP_ROLE_TIMER : function(callback){
        var cb = cc.callFunc(function(event){
            callback();
        });
        return cc.sequence(cc.delayTime(1),cb);
    },
    ...
    GP_WARNING_BLINK : function(delay){
        var blink = new cc.Blink(10,30);
        var fadeIn = cc.fadeTo(0.5,255);
        return cc.sequence(cc.delayTime(delay),fadeIn,blink);
    },
