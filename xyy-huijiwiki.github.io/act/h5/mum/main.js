var act_id = 46, isWx = false;
var v_main = null;
var canvas, context, canvasWidth = 640, canvasHeight = 1300;
var weixinData = {
...
var winSize = {
  ...
  resetImgData: function () {
    this.imdData = { top: 0, left: 0, scale: 1, rotate: 0, deltaX: 0, deltaY: 0 };
  },
  ...
  v_main = new Vue({
    ...
    methods: {
      ...
      chooseImage: function () {
        ...
        // }
        // else {
        //   // this.imgId = "res/test/3.jpg";
        //   // v_main.dialogs = "onLoadFaceModel";
        //   // setTimeout(function(){
        //   //   v_main.onDistanceImg();
        //   // },300)
        // }
        this.imgId = "res/test/1.jpg";
        v_main.dialogs = "onLoadFaceModel";
        setTimeout(function () {
          v_main.onDistanceImg();
        }, 300);
      ...
      onDistanceImg: async function (src) {
        ...
        if (detections.length == 0) {
          ...
          let resdouble = await faceapi.detectAllFaces(faceimg)
            .withFaceLandmarks(true)
            .withFaceDescriptors();
          let distance = await faceapi.round(faceapi.euclideanDistance(resdouble[0].descriptor, resdouble[1].descriptor));
          await v_main.getDistance(distance);
          console.log("相似度:" + v_main.caldistance + "% 结束识别,耗时:" + (new Date().getTime() - dt) / 1000 + "s");
        }
        this.dialogs = "";
      },
      ...
      getDistance: async function (distance) {
        ...
        caldistance /= 0.72;
        caldistance = caldistance > 1 ? 0.99 : caldistance < 0.3 ? 0.4 : caldistance;
...
function onListenerTouch(obj) {
  ...
  boxGesture.gesturescale = function (e) {
    if (v_main.touch) {
      ...
      document.getElementById("imgId").style.transform = "scale(" + winSize.imdData.scale + ") rotate(" + winSize.imdData.rotate + "deg)";
      // deltaScale = e.scale;
...
function setGesture(el) {
  ...
  document.addEventListener("touchmove", function (e) {
    ...
    if (e.touches.length >= 2 && istouch) {
      let scale = getTouchScale(now[0], now[1]) / getTouchScale(start[0], start[1]); //得到缩放比例，getDistance是勾股定理的一个方法
