var act_id = 47, isWx = false, xwz = false;
var v_main = null;
var canvas, context, canvasWidth = 640, canvasHeight = 1300, faceWidth = 200, faceHeight = 250;
var weixinData = {
...
var winSize = {
  ...
  resetImgData: function () {
    this.imdData.imgId_1 = { top: 0, left: 0, scale: 1, rotate: 0, deltaX: 0, deltaY: 0, bscale: 1, brotate: 0 };
    this.imdData.imgId_2 = { top: 0, left: 0, scale: 1, rotate: 0, deltaX: 0, deltaY: 0, bscale: 1, brotate: 0 };
  },
    ...
    methods: {
      onLoadFaceModel: async function () {
        ...
        if (!this.loadFaceModel) {
          ...
          console.log("初始化FaceModel完成,耗时:" + (new Date().getTime() - dt) / 1000 + "s");
        }
        ...
        if (isWx) {
          wx.chooseImage({
            ...
            success: function (res) {
              v_main["imgId_" + id] = res.localIds[0];
          ...
          if (id == 1)
            this.imgId_1 = "res/test_1.jpg";
          else
            this.imgId_2 = "res/test_4.jpg";
          v_main.dialogs = "onLoadFaceModel";
          setTimeout(function () {
            v_main.previewImage(id);
          }, 500);
      ...
      previewImage: async function (id) {
        ...
        if (detections.length == 1) {
          this.touch = false;
          let descriptor = await faceapi.detectAllFaces(faceimg)
      ...
      onDistanceImg: async function () {
        let distance = await faceapi.round(faceapi.euclideanDistance(this.descriptor[0], this.descriptor[1]));
      ...
      setImgPos: async function (id) {
        ...
        let scale = (width / height) < (winSize.userWidth / winSize.userHeight) ? scaleX : scaleY;
        img.css({
          "marginTop": -height >> 1,
          "marginLeft": -width >> 1,
      ...
      getDistance: async function (distance) {
        ...
        if (caldistance < 0.3)
          caldistance *= 2;
        else if (caldistance < 0.4)
          caldistance *= 1.75;
        else if (caldistance < 0.5)
          caldistance *= 1.72;
        else if (caldistance < 0.6)
          caldistance *= 1.55;
        else if (caldistance < 0.7)
          caldistance *= 1.45;
        else
          caldistance *= 1.38;
        caldistance = caldistance >= 1 ? 0.99 : caldistance < 0.2 ? 0.4 : caldistance;
      ...
      playVoice: function () {
        if (!this.playing) {
          this.playing = true;
          wx.playVoice({ localId: v_main.recordId });
          wx.onVoicePlayEnd({
            success: function (res) {
      ...
      uploadVoice: function () {
        if (this.recordId) {
          if (this.playing) {
            wx.stopVoice({ localId: v_main.recordId });
            this.playing = false;
          }
          if (this.sharedata) {
      ...
      onFinish: function () {
        this.dialogs = "onLoadFaceModel";
        if (this.sharedata) {
          if (this.sharedata.serverId_record == this.serverId_record && this.sharedata.theme == this.theme && this.sharedata.word == this.word && this.sharedata.caldistance == this.caldistance) {
            let equal = true;
            for (var item in this.sharedata.imdData) {
              for (var itemj in this.sharedata.imdData[item]) {
                if (this.sharedata.imdData[item][itemj] != winSize.imdData[item][itemj]) {
                  equal = false;
                  break;
                }
              }
      ...
      getImgData: function (ticket) {
        $.ajax({
          ...
          success: function (response) {
            ...
            document.getElementById("canvasUrlData").src = "upload/" + json.result.ticket + ".jpg";
...
var touchobj = {};
var start = [], moving = false, istouch = false;
function onListenerTouch(obj) {
  var box = document.getElementById(obj);
  var boxGesture = setGesture(box);
  boxGesture.gesturescale = function (e) {
    if (v_main.touch) {
      winSize.imdData["imgId_" + currTouch].scale = e.scale * winSize.imdData["imgId_" + currTouch].bscale * 1;
      winSize.imdData["imgId_" + currTouch].rotate = e.rotation * 1 + winSize.imdData["imgId_" + currTouch].brotate * 1;
      document.getElementById("imgId_" + currTouch).style.transform = "scale(" + winSize.imdData["imgId_" + currTouch].scale + ") rotate(" + winSize.imdData["imgId_" + currTouch].rotate + "deg)";
    }
  };
  ...
  el.addEventListener("touchmove", function (e) {
    ...
    if (istouch) {
      ...
      if (e.touches.length >= 2) {
        let scale = getTouchScale(now[0], now[1]) / getTouchScale(start[0], start[1]); //得到缩放比例，getTouchScale是勾股定理的一个方法
        let rotation = getTouchAngle(now[0], now[1]) - getTouchAngle(start[0], start[1]);  //得到旋转角度，getAngle是得到夹角的一个方法
  ...
  el.addEventListener("touchend", function (e) {
    if (istouch) {
      ...
      if (!moving) {
        ...
        winSize.imdData["imgId_" + currTouch].bscale = winSize.imdData["imgId_" + currTouch].scale * 1;
        winSize.imdData["imgId_" + currTouch].brotate = winSize.imdData["imgId_" + currTouch].rotate * 1;
      }
    }
  }, false);
  return touchobj;
  ...
  $.ajax({
    ...
    success: function success(res) {
      ...
      wx.config({
        ...
        jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', 'chooseImage', 'previewImage', 'uploadImage', 'downloadImage', 'startRecord', 'stopRecord', 'onVoiceRecordEnd', 'playVoice', 'pauseVoice', 'stopVoice', 'onVoicePlayEnd', 'uploadVoice', 'downloadVoice']
      });
    }
  });
  wx.ready(function () {
    ...
    if (getQueryString("c") != null && v_main.sharedata.serverId_record) { //有录音
